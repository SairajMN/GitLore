"""Groq provider adapter — fastest and cheapest for classification tasks."""
...
class GroqProvider(LLMProvider):
    ...
    async def generate_text(self, options: GenerateTextOptions) -> GenerateTextResult:
        model = options.model or "llama-3.1-8b-instant"
        ...
        logger.info("groq.generate_text model=%s latency_ms=%d tokens=%s", model, latency_ms, tokens)
        return GenerateTextResult(text=text, model=model, provider="groq", latency_ms=latency_ms, tokens_used=tokens)
    ...
    async def extract_structured(self, options: StructuredExtractOptions) -> StructuredExtractResult:
        ...
        try:
            ...
            logger.warning("groq.extract_structured: failed to parse JSON, returning empty dict")
            parsed = {}

        return StructuredExtractResult(data=parsed, model=result.model, provider="groq")