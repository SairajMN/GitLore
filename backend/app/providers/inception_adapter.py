"""Inception Labs provider adapter — specialized models for synthesis."""
...
class InceptionProvider(LLMProvider):
    ...
    def is_available(self) -> bool:
        return bool(self._settings.inception_api_key)

    async def generate_text(self, options: GenerateTextOptions) -> GenerateTextResult:
        model = options.model or "inception-v1"
        url = f"{self._settings.inception_base_url}/chat/completions"
        headers = {
            "Authorization": f"Bearer {self._settings.inception_api_key}",
        ...
        logger.info("inception.generate_text model=%s latency_ms=%d tokens=%s", model, latency_ms, tokens)
        return GenerateTextResult(text=text, model=model, provider="inception", latency_ms=latency_ms, tokens_used=tokens)
    ...
    async def extract_structured(self, options: StructuredExtractOptions) -> StructuredExtractResult:
        ...
        try:
            ...
            logger.warning("inception.extract_structured: failed to parse JSON")
            parsed = {}

        return StructuredExtractResult(data=parsed, model=result.model, provider="inception")